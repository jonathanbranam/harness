...
- **WHEN** the agent calls `read` on a path that resolves outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the offending path

#### Scenario: Attempted write outside workspace
- **WHEN** the agent calls `write` or `edit` on a path that resolves outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the offending path

#### Scenario: Attempted ls/find/grep outside workspace
- **WHEN** the agent calls `ls`, `find`, or `grep` with a `path` argument that resolves outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the offending path
...
#### Scenario: Path inside temp directory succeeds
- **WHEN** the agent calls `read`, `write`, `edit`, `ls`, `find`, or `grep` on a path that resolves inside the harness-defined temporary directory
- **THEN** the tool call proceeds normally

### Requirement: Symlink escapes are blocked
A path that resolves lexically inside one of the session's allowed roots (the workspace directory or the temporary directory) but, once symlinks are followed (via `realpath`), points outside all of them SHALL be blocked — for both the path-jailed tools (`read`/`write`/`edit`/`ls`/`find`/`grep`) and paths referenced in `bash` commands. For a target that doesn't exist yet (e.g. a `write` destination), the check resolves the nearest existing ancestor directory's real path instead.

#### Scenario: Read through a symlink pointing outside the workspace
- **WHEN** the agent calls `read` on a path inside an allowed root that is, or passes through, a symlink resolving outside all allowed roots
- **THEN** the tool call is blocked with a reason identifying the offending path
...
- **WHEN** the agent runs a `bash` command that changes directory outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the workspace violation

#### Scenario: Absolute path outside workspace
- **WHEN** the agent runs a `bash` command referencing an absolute path outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the workspace violation

#### Scenario: Relative traversal outside workspace
- **WHEN** the agent runs a `bash` command whose arguments contain a `..` traversal sequence that would resolve outside every allowed root
- **THEN** the tool call is blocked with a reason identifying the workspace violation
...
#### Scenario: cd into the temp directory succeeds
- **WHEN** the agent runs a `bash` command that changes directory into the harness-defined temporary directory
- **THEN** the tool call proceeds normally
...
- **THEN** the system prompt text includes both the workspace directory path and the temporary directory path, and an instruction not to access anything outside them
