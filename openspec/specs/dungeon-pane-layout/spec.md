# Dungeon Pane Layout Specification

## Purpose

Defines resizable, focusable pane layout for the dungeon harness's board and chat panes, so a user can widen, minimize, or maximize either instead of being stuck with a fixed-width split.

## Requirements

### Requirement: Draggable pane resize
The dungeon page SHALL let the user resize the board and chat panes by dragging a handle on the border between them.

#### Scenario: User drags the resize handle
- **WHEN** the user presses and drags the handle between the board pane and the chat pane
...
The board pane SHALL default to a wider width than the chat pane on initial load.

#### Scenario: Fresh page load
- **WHEN** the dungeon page loads with no prior layout adjustment in the session
- **THEN** the board pane occupies more horizontal space than the chat pane
...
- **THEN** the chat pane expands to fill the freed space and the board pane collapses to its minimized rail
...
- **THEN** the chat pane collapses to a narrow rail with a 90°-rotated title, and the board pane grows to fill the freed width
