...
- **WHEN** the agent starts an assistant message, makes one or more tool calls, and ends the message without ever emitting a text delta
...
- **WHEN** the agent emits at least one text delta as part of an assistant message that also includes tool calls
...
- **WHEN** the agent makes several tool calls in a row within a single turn, with no text emitted between them
