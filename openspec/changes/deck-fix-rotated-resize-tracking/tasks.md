...
- [ ] 1.1 In `client-deck/src/components/DeckCanvas.tsx`'s
...
- [ ] 1.2 In `onMove`, after `width`/`height` are finalized (including any
...
- [ ] 1.3 Feed the resulting `{ x, y, width, height }` into the existing
...
- [ ] 2.1 Using `playwright-cli` against the already-running
...
- [ ] 2.2 Repeat the same corner-handle drags on an unrotated object (or
...
- [ ] 2.3 Repeat a rotated-object corner drag while holding Shift
      (aspect-ratio lock) and confirm the anchor corner still stays visually
      fixed while width/height maintain the locked ratio.
- [ ] 2.4 Confirm a rotated-object corner drag that would push the object
