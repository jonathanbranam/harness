...
**Reason**: The fixed terrain vocabulary (`plains`/`forest`/`water`/`stone`)
tied the board to game-rule concepts the harness no longer models.
**Migration**: Use `dungeon_set_cell_fill` to set any color on a cell
instead.

### Requirement: Units are placed from a fixed archetype catalog
**Reason**: The harness no longer enforces or reproduces unit stats or
game rules; the fixed archetype catalog and its stat table are dropped
along with all game-rule computation.
**Migration**: Use `dungeon_draw_shape` to place a labeled circle or
rectangle representing a unit, with whatever color and label the scenario
calls for.

### Requirement: Units can be removed from the board
**Reason**: `dungeon_remove_unit` is replaced by a generic, kind-agnostic
remove tool that works on any drawn object.
**Migration**: Use `dungeon_remove_object` with the object's id.

### Requirement: Movement preview computes a range-limited path
**Reason**: The harness no longer computes movement range, pathing, or
occupancy-based blocking — those are game rules it should not reimplement.
**Migration**: None. If a scenario needs to show a path, draw it directly
with `dungeon_draw_line`.

### Requirement: Movement can be committed to reposition a unit
**Reason**: `dungeon_move_unit`'s range/occupancy validation is game-rule
logic the harness no longer performs.
**Migration**: Use `dungeon_move_object` to reposition any drawn object
directly, with no range or occupancy check.

### Requirement: Attack preview computes footprint and penetration
**Reason**: Attack footprint/penetration computation is game-rule logic
the harness no longer reproduces.
**Migration**: Use `dungeon_draw_overlay` to highlight whatever cells the
scenario intends as the footprint, and `dungeon_draw_shape`/
`dungeon_draw_label` to mark which are hit.

### Requirement: Board canvas renders terrain, units, and previews
**Reason**: This requirement described rendering an archetype-specific
terrain enum, unit glyphs, and auto-computed movement/attack previews —
none of which the harness still produces. Rendering is now driven by
generic drawn objects and free-form cell fill colors instead.
**Migration**: See the new "Board canvas renders cell fill and drawn
objects" requirement.
