...
Every content-mutating operation on the deck — object add/remove and all
`presentation_update` actions (`setPosition`, `setSize`, `setText`,
`setFillColor`, `setFontColor`, `setBorderColor`, `setFontSize`,
`applyGridLayout`, `addObject`, `removeObject`, `applyTextStyle`), slide
add/remove, and deck create/delete — SHALL push one entry onto a single
