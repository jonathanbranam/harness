...
function Block({ block, muted }: { block: ContextBlock; muted: boolean }) {
  if (block.role === 'tool') {
    ...
    return (
      <div
        className={`text-xs font-mono border border-gray-300 dark:border-gray-700 rounded-md px-2 py-1 ${
          muted ? 'bg-gray-50 text-gray-400 dark:bg-gray-900/50 dark:text-gray-500' : 'bg-gray-100 text-gray-700 dark:bg-gray-800/60 dark:text-gray-300'
        }`}
      >
  ...
  return (
    <div
      className={`rounded-lg px-3 py-2 text-sm border ${block.role !== 'assistant' ? 'whitespace-pre-wrap' : ''} ${
        muted
          ? 'bg-gray-50 border-gray-200 text-gray-400 dark:bg-gray-900/50 dark:border-gray-800 dark:text-gray-500'
...
export function ApparatusView({
  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-900 dark:bg-gray-950 dark:text-white">
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 p-4 bg-gray-50 dark:bg-gray-900/50">
        <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Foundation</h3>
        <div className="text-xs text-gray-600 dark:text-gray-500 mb-2 line-clamp-3">{foundation.systemPrompt || 'No system prompt loaded'}</div>
        {foundation.skills.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {foundation.skills.map((skill) => (
              <span key={skill.name} className="px-2 py-0.5 rounded bg-gray-200 text-gray-700 dark:bg-gray-800 dark:text-gray-300 text-xs">
                {skill.name}
              </span>
            ))}
          </div>
