...
tmux send-keys "$DIR/scripts/set-pane-title.sh client-introspect && npm run dev:client-introspect" Enter

# Return to original (top-left) pane, then split bottom-left for introspect server
tmux select-pane -t "$ORIGIN"
tmux split-window -v -c "$DIR"
tmux send-keys "$DIR/scripts/set-pane-title.sh introspect-harness-server && npm run dev:introspect" Enter
