 ...
 * iterates this list, so the board pane the rebuilt bench adds back only
 * needs an entry here plus a rendered `<Panel>`. It held two entries until
 * the freehand board was removed (docs/dungeon-harness/backout-plan.md). */
const PANE_IDS = [CHAT_PANE] as const
type PaneId = (typeof PANE_IDS)[number]
const PANE_TITLES: Record<PaneId, string> = { [CHAT_PANE]: 'Chat with pi' }
const DEFAULT_SIZES: Record<PaneId, number> = { [CHAT_PANE]: 100 }
